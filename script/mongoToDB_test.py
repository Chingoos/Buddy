...
for v in db.places_detail.find():
  if (v['location'].get('address', 'Null') != 'Null') and  (v['location']['address'].get('district','Null') != 'Null'):
    ...
    x.execute("""INSERT INTO places_test(place_id, name, city, state, zip_code, country,  latitude, longitude, as_of_date) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, NOW())""", (v['placeId'], v['name'], v['location']['address']['city'],  v['location']['address']['state'],  v['location']['address']['postalCode'],  v['location']['address']['country'],  v['location']['position'][0], v['location']['position'][1]))
